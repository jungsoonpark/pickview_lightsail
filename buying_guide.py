...
def generate_buying_guide(keyword):
    try:
        ...
        # 응답 로그 추가
        logging.info(f"GPT 응답: {response}")
        
        guide = response['choices'][0]['message']['content']  # 응답을 직접 문자열로 처리
        logging.info(f"응답 내용: {guide}")  # 응답 내용 확인
        
        if not guide or "여기에 GPT 응답 문자열을 넣어주세요" in guide:
            logging.error(f"GPT 응답 오류: 예상치 못한 응답 내용입니다. 응답을 다시 확인해 주세요.")
            return None
